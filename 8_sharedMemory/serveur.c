...
Utilisation: 
	- ./serveur
	- avec ./cuisinier dans une autre fenêtre
-----------------------------------------
*/
...
int main(int argc, char *argv[]){
	...
	// boucle pour servir les pizzas
	for(i=0;i<MAX_PIZZA;i++){
		sem_wait(NoPizza); //verification qu'il y a au moins une pizza sur l'étagère à servir
		sleep(rand()%MAX_TEMPS_ATTENTE+1);//un temps d'attente aléatoire pour servir une pizza
		sem_wait(aMonTour);//bloquer le cuisinier pendant le temps qu'il sert
		(*etagere)--;
		printf("Serveur: J'ai servi une pizza, il y a %d pizzas sur l'étagère.\n", *etagere);
		sem_post(aMonTour);//debloquer le cuisinier
		sem_post(TropDePizza);//le cuisinier peut cuisiner une pizza de plus
	
	}

	printf("Serveur: J'ai servi les 10 pizzas, il y a %d pizzas à servir.\n", *etagere);
